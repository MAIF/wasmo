# Contributing to wasmo

These guidelines apply to all wasmo projects living in the the `MAIF/wasmo` repository.
...
- `cli`: source code of the Wasmo CLI
- `docs`: contains the documentation of this project
- `server`: express server to handle plugin management and expose API to discuss with the CLI
- `ui`:  custom UI, written with React.JS, contains the source code for creating and editing plugins directly in a UI
...
1. To avoid duplicated effort, it might be good to check the [issue tracker](https://github.com/MAIF/wasmo/issues) and [existing pull requests](https://github.com/MAIF/wasmo/pulls) for existing work.
   - If there is no ticket yet, feel free to [create one](https://github.com/MAIF/wasmo/issues/new) to discuss the problem and the approach you want to take to solve it.
1. [Fork the project](https://github.com/MAIF/wasmo#fork-destination-box) on GitHub. You'll need to create a feature-branch for your work on your fork, as this way you'll be able to submit a pull request against the mainline wasmo.
...
1. If it's a new feature, or a change of behaviour, document it on the [wasmo docs](https://github.com/MAIF/wasmo/tree/main/manual), remember, an undocumented feature is not a feature.
...
You must add the dependency and its licence in https://github.com/MAIF/wasmo/blob/main/licences.md

## Documentation

if you add features to wasmo, don't forget to modify the user documentation


## Before running

```
git clone git@github.com:MAIF/wasmo.git
```
...
```
cd wasmo/server
nano .env # configure the S3 credentials if you want to use the UI
yarn 
node index.js
```
